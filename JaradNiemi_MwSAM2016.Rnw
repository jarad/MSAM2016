...
\begin{frame}
\frametitle{Hierarchical probit regression model}
...
\[ 
M_{ik} \ind Ber(\pi_{ik}) \quad \pi_{ik} = \Phi(\zeta_{ik}) \quad \zeta_{ik} = w_{ik}^\top \alpha_k.
\]
...
M_{ik} &\ind Ber(\pi_{ik})    & \pi_{ik}     &= \Phi\left(\omega_{ik}^\top \alpha\phantom{_k}  \right) \\
...
<<informative_missing_data, fig.width=10, out.width="\\textwidth">>=
library(dplyr)

d = readr::read_csv("from_dennis/BdrawsIM.csv")[101:10100,-1]
...
<<informative_missing_plot, dependson="informative_missing_data", fig.width=10, out.width="\\textwidth">>=
...
<<name_reveal, dependson="informative_missing_data", fig.width=10, out.width="\\textwidth">>=
ggplot(p_d, aes(dist, probability, color=Surface)) +
#  geom_point(data=fg_dist2, aes(dist, p, size=n)) +
  geom_line(aes(group=Surface)) +
  facet_grid(kicker_name ~.) +
  theme_bw() +
  labs(x="Distance (yards)",y="Probability of making the kick", size="Number of\nattempts") +
  ylim(0,1) +
  scale_color_manual(values = c("Grass" = "seagreen", "Synthetic" = "Black"))

...
<<count_table, results="asis">>=
fg_table %>%
  xtable(digits = c(1,NA,0,0,2)) %>%
  print(include.rownames = FALSE)
@

In addition, we can ask why the first kicker took so few kicks. 
...
#  binomial_smooth(se=FALSE) +
  ...
  theme_bw() +
  labs(x="Distance", y="Proportion made", size="Number of\nattempts")
@
...
\item $X_{ak}$ be a vector of explanatory variables, e.g. 
\end{itemize}

\vspace{0.1in} \pause

The logistic regression model for each kicker $k$ assumes 
\[ 
...
glm_predictions <- function(d) {
  ...
  dd <- expand.grid(dist=17:70, Surface=c("Grass","Synthetic"))
...
ggplot(fg_logistic_by_kicker, aes(dist, probability)) +
  geom_point(data=fg_dist, aes(dist, p, size=n)) +
  geom_line(aes(color=Surface, group=Surface)) +
  facet_grid(kicker_hidden ~.) +
  theme_bw() +
  labs(x="Distance",y="Probability of making the kick")
@
...
\]
\pause 
but then we assume a distribution for the $\beta_k$, i.e. 
\[ 
\beta_k \ind N(\mu,\Sigma).
\]
\pause
The impact of this distribution is to learn about the 
\begin{itemize}
...
\end{itemize}
\pause
If a diagonal element of $\Sigma$ is estimated to be 
\begin{itemize}
\item small then kickers are similar and we borrow a lot of information across kickers
\item large then kickers are dissimilar and we do not borrow much information.
\end{itemize}
\end{frame}
...
% \begin{figure}[!ht]
% \centering
%     \includegraphics[scale=0.33,clip]{Histsd}
